...
/* set column headers at first line */
        if(!aoa[0]) aoa[0] = [];
            aoa[0][0]='Курс';
/* fill data to array */
        for(var i = 0; i < this.eduprog_courseArray.length; ++i) {
            if(!aoa[i+1]) aoa[i+1] = [];
            aoa[i+1][0]=this.eduprog_courseArray[i].Course_name;
        }
		/* generate worksheet */
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'eduprog_course');
        

        wb.Props = {
            Title: "Программы обучения::Курсы в программе",
            Subject: "Программы обучения::Курсы в программе",
            Company: "master.bami",
		...
		/* save to file */
		XLSX.writeFile(wb, 'eduprog_course.xlsx');
	}
    backToList() {
        this.opened = false;
